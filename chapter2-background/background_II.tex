...
Time difference EIT is sensitive to the movement of fluis within the body 
...
Under this assumption the derivitave components of \fref{eq:farad} and \fref{eq:amp}
...
$ \Delta \cdot (\Delta \times \vec{H}) = 0 $ with \fref{eq:amp2}, we get
\begin{equation} \label{eq:mid1}
	\Delta \cdot \vec{J} = 0.
\end{equation}
Combining \fref{eq:mid1} with Ohm's Law (\fref{eq:ohm}) we get 
\begin{equation} \label{eq:mid2}
	\Delta \cdot (\sigma \vec{E}) = 0.
\end{equation}
And finally with \fref{eq:field} relating the electric field and potential 
we get 
\begin{equation}
...
\TODO{ADD overview from whereever I put it!}
\subsubsection{Motion Correction} \label{sec:motion_correction}
\TODO{Just background}

\section{Summary}
\TODO{bring it back to the general motivation briefly}
