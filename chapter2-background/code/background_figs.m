...
if 1
	...
	a.LineStyle = 'none';
	a.Parent = f.CurrentAxes;  % associate annotation with current axes
	a.Position = [elec_pt(1,1)+1.185,elec_pt(1,2)+0.08,0.15,0.15];
	%annotation('arrow',xa,ya);
	% 
	[stim,meas_sel] = mk_stim_patterns(16,1,[0,1],[0,1],{'no_meas_current'}, 1);
	fmdl.stimulation = stim;
...
%%%%% FIGURE 2 - the stim meas patterns!!
%	% Make lines for each electrode
f = figure(2);
set(gcf,'renderer','painters');
set(groot,'defaultAxesTickLabelInterpreter','latex');  
...
plot([elec_pt(1,1),elec_pt(1,1),elec_pt(1,1)+0.045],[elec_pt(1,2),elec_pt(1,2)+.15,elec_pt(1,2)+.15],'k','lineWidth',2)
plot([elec_pt(2,1)-.045,elec_pt(2,1),elec_pt(2,1),elec_pt(2,1)], ...
	[elec_pt(1,2)+.15 ,elec_pt(1,2)+.15 ,elec_pt(2,2)     ,elec_pt(2,2)],'k','lineWidth',2)
xa = [elec_pt(1,1)+.1,elec_pt(1,1)+.25];
ya = [elec_pt(1,2)+.10,elec_pt(1,2)+.10];
% now you can use data units
a = annotation('arrow');
a.Parent = f.CurrentAxes;  % associate annotation with current axes
...
a = annotation('ellipse');
a.Parent = f.CurrentAxes;  % associate annotation with current axes
a.Position = [elec_pt(1,1)+.042,elec_pt(1,2)+0.025,0.25,0.25];
a.LineWidth = 2;
a = annotation('textbox','interpreter','latex');
a.FontSize = 18;
a.String = '\emph{I}';
a.LineStyle = 'none';
a.Parent = f.CurrentAxes;  % associate annotation with current axes
a.Position = [elec_pt(1,1)+0.13,elec_pt(1,2)+0.08,0.15,0.15];
%annotation('arrow',xa,ya);
% 
[stim,meas_sel] = mk_stim_patterns(16,1,[0,1],[0,1],{'no_meas_current'}, 1);
fmdl.stimulation = stim;
...
img.elem_data(fmdl.mat_idx{3})= 0.3; % llung
img_v = img;
% Stimulate between elecs 16 and 1 to get more interesting pattern
img_v.fwd_model.stimulation(1).stim_pattern = sparse([16;1],1,[1,-1],16,1);
img_v.fwd_solve.get_all_meas = 1;
vh = fwd_solve(img_v);
img_v.fwd_model.mdl_slice_mapper.npx = 1000;
img_v.fwd_model.mdl_slice_mapper.npy = 1000;
...
set(gcf,'Position',[ 180         643        1423         627])

print('../imgs/common_stim_meas_patterns', '-dsvg') 